...
class UUIDIndex(Base):
    ...
    def __database_init__(db_session: scoped_session):
        ...
        for entry in results[db_config.table_names.uuid_index]:
            db_session.add(UUIDIndex(**{key: UUID(entry[key]) if key.__contains__("uuid") else entry[key] for key in
...
class TableIndex(Base):
    ...
    __parent_name__ = db_config.table_names.uuid_index
    ...
    def __database_init__(db_session: scoped_session):
        ...
        for entry in results[db_config.table_names.table_index]:
            db_session.add(TableIndex(**{key: UUID(entry[key]) if key.__contains__("uuid") else entry[key] for key in
                                         ["uuid", "name", "model"]}))
        db_session.commit()
        return True
